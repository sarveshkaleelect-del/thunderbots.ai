...
function StatusPill({ workflowId }: { workflowId: string }) {
  ...
  const tone = isLive ? 'success' : data.status === 'error' || data.status === 'invalid_token' ? 'danger' : 'warning'
  const label = isLive ? 'Live' : data.status === 'invalid_token' ? 'Invalid token' : data.status

  return <Badge tone={tone} dot={isLive}>{label}</Badge>
}

export default function TelegramIndexPage() {
  ...
  return (
    <div className="tb2-shell">
      <SubPageBar crumb="Telegram" crumbIcon={<Send size={13} className="text-sky-400/70" />} />

      <div className="max-w-2xl mx-auto px-6 py-10 space-y-6">
        <div className="tb2-rise">
          <h1 className="text-xl font-bold text-white">Telegram Channel</h1>
          <p className="text-sm text-white/35 mt-1">
            Connect any chatbot to its own Telegram Bot via the official Telegram Bot API.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(workflows as WorkflowListItem[]).map((wf, i) => (
              <button
                key={wf.id}
                onClick={() => router.push(`/telegram/${wf.id}`)}
