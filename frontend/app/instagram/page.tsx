...
import { Instagram, Bot, ChevronRight } from 'lucide-react'
...
function StatusPill({ workflowId }: { workflowId: string }) {
  ...
  const tone = isLive ? 'success' : data.status === 'error' || data.status === 'expired' ? 'danger' : 'warning'
...
export default function InstagramIndexPage() {
  ...
  return (
    <div className="tb2-shell">
      <SubPageBar crumb="Instagram" crumbIcon={<Instagram size={13} className="text-pink-400/70" />} />

      <div className="max-w-2xl mx-auto px-6 py-10 space-y-6">
        <div className="tb2-rise">
          <h1 className="text-xl font-bold text-white">Instagram Channel</h1>
          <p className="text-sm text-white/35 mt-1">
            Connect any chatbot to an Instagram Business account via Meta and reply to DMs automatically.
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {(workflows as WorkflowListItem[]).map((wf, i) => (
              <button
                key={wf.id}
                onClick={() => router.push(`/instagram/${wf.id}`)}
